...
// Para la impresión del acta, permitiremos que sea vista por el profesor que la envió o un admin.
// Puedes ajustar esta lógica de seguridad según tus necesidades.
...
try {
    $stmt_info = $pdo->prepare("
        SELECT
            ...
            p.nombre_completo AS nombre_profesor_asignado
        FROM asignaturas a
        JOIN horarios h ON a.id = h.id_asignatura
        JOIN semestres s ON h.id_semestre = s.id
        JOIN anios_academicos aa ON s.id_anio_academico = aa.id
        LEFT JOIN profesores pr ON h.id_profesor = pr.id
        LEFT JOIN usuarios p ON pr.id_usuario = p.id
        WHERE h.id_asignatura = :asignatura_id
          AND h.id_semestre = :semestre_id
          AND s.id_anio_academico = :anio_academico_id
    ...
    // OBTENER ESTUDIANTES Y SUS NOTAS (Solo las notas que estén APROBADA_ADMIN)
    $stmt_notas = $pdo->prepare("
        ...
        WHERE ie.id_asignatura = :asignatura_id
          ...
          AND ie.confirmada = 1
          AND n.estado_envio_acta = 'APROBADA_ADMIN' -- SOLO NOTAS APROBADAS POR EL ADMINISTRADOR
...
<body>
  <div class="container-a4">
    <div class="header-acta">
      <?php if (!empty($logo)): ?>
        <img src="<?= $logo ?>" alt="Logo UNGE" class="rounded">
      <?php else: ?>
        <div style="font-size: 40px;"><i class="fas fa-university"></i></div>
      <?php endif; ?>
      <h1>ACTA DE CALIFICACIONES FINALES</h1>
      <h2>Universidad Nacional de Guinea Ecuatorial (UNGE)</h2>
      <h3>Facultad de Ciencias Económicas · Departamento de Informática de Gestión</h3>
      <hr />
    </div>
    ...
    <?php if (!empty($notas_estudiantes)): ?>
      <div class="table-responsive">
        <table class="table table-bordered table-notas">
          ...
          <tbody>
            <?php foreach ($notas_estudiantes as $index => $nota): ?>
              <tr>
                <td><?= $index + 1 ?></td>
                <td><?= htmlspecialchars($nota['nombre_estudiante']) ?></td>
                <td><?= number_format($nota['nota'], 2) ?></td>
                <td class="<?= $nota['nota'] >= 5 ? 'text-success' : 'text-danger' ?>">
                  <?= $nota['nota'] >= 5 ? 'APROBADO' : 'REPROBADO' ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
    ...
    <?php else: ?>
      <div class="alert alert-warning text-center">No hay notas aprobadas por el administrador para esta asignatura.</div>
    <?php endif; ?>
